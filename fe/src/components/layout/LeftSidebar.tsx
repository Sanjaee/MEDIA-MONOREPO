...
import { MessagesSidebarLink } from "./MessagesSidebarLink";
  ...
  return (
    <aside className="hidden sm:flex flex-col w-20 xl:w-64 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto p-4">
      <nav className="flex flex-col gap-2">
        ...
        {session?.user && (
          <Link href={`/${(session.user as any).username}`} className="flex items-center gap-4 p-3 w-fit rounded-full hover:bg-accent transition">
